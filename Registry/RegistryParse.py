...
class RegistryBlock(object):
    ...
    def unpack_string(self, offset, length):
        ...
        """
        b = struct.unpack_from("<%ds" % (length), self._buf, self._offset + offset)[0]
...
class VKRecord(Record):
    ...
    def data(self):
        ...
        if data_type == RegSZ or data_type == RegExpandSZ:
            ...
            try:
                # In Python 3, we simply decode the bytes to string directly
                s = s.decode("utf-16le") 
...
class NKRecord(Record):
    ...
    def classname(self):
        ...
        b = struct.unpack_from("<%ds" % (classname_length), self._buf, d.data_offset())[0]
        return b.decode('latin-1', 'replace')
